...
class UpdateTemplateComponent extends Component { //updated 
    constructor(props) {
        super(props)
        // console.log('props hain ', this.props.match.params.id, this.props.match.params.employeeId);//new added
        const {employeeId, id} = this.props.match.params; //updated line
        this.state = {
            employeeId: employeeId,
            id: id,
            templateDay: "",
    ...
    saveEmployee = (e) => {
        ...
        const checkInObj = new Date(`2023-08-04T${this.state.checkIn}`).toLocaleTimeString([], {hour12: false, hour: '2-digit', minute: '2-digit'});
        const checkOutObj = new Date(`2023-08-04T${this.state.checkOut}`).toLocaleTimeString([], {hour12: false,  hour: '2-digit', minute: '2-digit'});
        const breakStartObj = new Date(`2023-08-04T${this.state.lunchBreakStart}`).toLocaleTimeString([], {hour12: false,  hour: '2-digit', minute: '2-digit' });
        const breakEndObj = new Date(`2023-08-04T${this.state.lunchBreakEnd}`).toLocaleTimeString([], {hour12: false, hour: '2-digit', minute: '2-digit'});


        let template = {
                        templateDay: this.state.templateDay, 
                        checkIn: checkInObj,
                        checkOut: checkOutObj,
                        lunchBreakStart: breakStartObj,
                        lunchBreakEnd: breakEndObj
...
export default withRouter(UpdateTemplateComponent); //added bcz withRouter mei wrap krna hai